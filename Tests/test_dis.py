...
def test_dis_with_offsets():
    ...
    with contextlib.redirect_stdout(f):
        dis(test_f, True)
    assert "ldarg.1" in f.getvalue()
    assert "// 0 LOAD_CONST - 1 ((1, 2, 3, 4))" in f.getvalue()

def test_dis_with_no_pc():
    def test_f():
        numbers = (1, 2, 3, 4)
        return sum(numbers)
...
def test_dis_native():
    ...
    with contextlib.redirect_stdout(f):
        dis_native(test_f)
    assert "PUSH RBP" in f.getvalue()

@unittest.skipIf(sys.platform.startswith("win"), "no windows support yet")
def test_dis_native_with_offsets():
    def test_f():
        numbers = (1, 2, 3, 4)
        return sum(numbers)
    ...
    print(f.getvalue())
    assert "PUSH RBP" in f.getvalue()
    assert "; 10 RETURN_VALUE - None (None)" in f.getvalue()
    assert "; METHOD_PYTUPLE_NEW" in f.getvalue()
